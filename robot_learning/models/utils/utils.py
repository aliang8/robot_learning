...
def make_mlp(
    ...
    for hidden_dim in net_kwargs.hidden_dims:
        ...
        if net_kwargs.dropout > 0.0:
            mlp.append(nn.Dropout(net_kwargs.dropout))
...
class ConvNet(nn.Module):
    ...
    def forward(self, x, return_intermediates: bool = False):
        ...
        if original_input.ndim == 5:  # [B, T, C, H, W]
            # combine TC together
            x, ps = einops.pack([original_input], "B * H W")
