...
def load_trajectory_data(traj_dir: Path) -> Tuple[List[Path], List[Path], List[dict]]:
    ...
    images0 = sorted(
        list(traj_dir.glob("images0/*")), key=lambda x: int(x.stem.split("_")[-1])
    )
    images1 = (
        sorted(
            list(traj_dir.glob("images1/*")),
            key=lambda x: int(x.stem.split("_")[-1]),
        )
        if (traj_dir / "images1").exists()
        else None
    )

    images2 = (
        sorted(
            list(traj_dir.glob("images2/*")),
            key=lambda x: int(x.stem.split("_")[-1]),
        )
        if (traj_dir / "images2").exists()
        else None
    )
    ...
    return images0, images1, images2, depth_imgs, metadata
...
def visualize_trajectory(data_dir: Path, output_path: Path = None, fps: int = 30):
    ...
    for traj_idx, traj_dir in enumerate(tqdm.tqdm(traj_dirs[:1])):
        images0, images1, images2, depth_imgs, metadata = load_trajectory_data(traj_dir)
        for i, img0_path in enumerate(images0):
            img1_path = images1[i] if images1 and i < len(images1) else None
            img2_path = images2[i] if images2 and i < len(images2) else None
            depth_img_path = (
    ...
    img0_path, img1_path, img2_path, depth_img_path = all_images[0]
    ...
    def update(frame):
        ...
        img0_path, img1_path, img2_path, depth_img_path = all_images[frame]
